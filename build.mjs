...
// Env for the website build: SUPABASE_URL, SUPABASE_ANON_KEY, SITE_URL (all optional; without them the app runs in local-only mode).
...
const appJs=["src/cat.js","src/symbols.js","src/view3d.js","src/app.js"].map(read).join("\n");
...
if(mode==="artifact"){
  ...
  const html=`<!DOCTYPE html><html lang="en"><head>${META}\n${headPart}\n</head><body style="margin:0">\n${bodyPart}\n<script>\n${read("vendor/three.min.js")}\n</script>\n${inl("jspdf-src","vendor/jspdf.min.js")}\n${inl("svg2pdf-src","vendor/svg2pdf.min.js")}\n${inl("pdfjs-src","vendor/pdf.min.js")}\n${inl("pdfjs-worker-src","vendor/pdf.worker.min.js")}\n<script>\n${appJs}\n</script></body></html>`;
  ...
  copy("src/cloud.js",D,"app/cloud.js");
  for(const f of ["three.min.js","jspdf.min.js","svg2pdf.min.js","pdf.min.js","pdf.worker.min.js"])copy("vendor/"+f,D,"app/vendor/"+f);
  copy("node_modules/@supabase/supabase-js/dist/umd/supabase.js",D,"app/vendor/supabase.js");
  // landing
  out(D,"index.html",read("site/index.html").replace("__KS_CONFIG__",JSON.stringify(cfg)));
  fs.cpSync(path.join(ROOT,"public"),path.join(ROOT,D),{recursive:true});
  out(D,"_redirects",["/k/*  /app/index.html  200","/app  /app/index.html  200","/app/  /app/index.html  200"].join("\n")+"\n");
  out(D,"_headers","/app/vendor/*\n  Cache-Control: public, max-age=31536000, immutable\n");
  console.log("dist/ built",cfg.supabaseUrl?"with Supabase config":"in local-only mode (no SUPABASE_URL)");
}
